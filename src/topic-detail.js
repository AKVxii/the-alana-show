...
if (!topic) {
  ...
  renderTopicEpisodes(matchingTopicEpisodes(editorialEpisodes));
  loadTopicEpisodes();
}

function matchingTopicEpisodes(source) {
  return source
...
function renderTopicEpisodes(episodes) {
...
async function loadTopicEpisodes() {
  ...
  try {
    ...
    const episodes = matchingTopicEpisodes(Array.isArray(payload.episodes) ? payload.episodes : []);
    ...
    renderTopicEpisodes(episodes);
  } catch {
    status.textContent = "The live conversation list is temporarily unavailable.";
    grid.insertAdjacentHTML("afterbegin", `<div class="media-empty"><h3>Continue in the archive.</h3><p>The topic page remains available while the live feed reconnects.</p><a class="button button-gold" href="/episodes?topic=${encodeURIComponent(topic.name)}">Browse ${escapeHtml(topic.name)} conversations</a></div>`);
  }
}
