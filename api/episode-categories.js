...
const TITLE_FALLBACK_OVERRIDES = [
  { pattern: /\bsenjutsu\b/i, categories: ["Wellness", "Leadership"] },
  { pattern: /\bpurpose-driven leadership\b.*\bashley vertuno\b/i, categories: ["Leadership", "Wellness"] },
  { pattern: /\bequipping the next generation of leaders\b/i, categories: ["Leadership", "Community"] },
  { pattern: /\bmichael barnett\b/i, categories: ["Public Service", "Leadership"] },
  { pattern: /\bwe fund the blue\b|\bmaca\b/i, categories: ["Community", "Public Service", "Stepping Up"] }
];
...
function categorizeEpisode(episode = {}) {
  ...
  const titleOverride = TITLE_FALLBACK_OVERRIDES.find(entry => entry.pattern.test(title));
  const overrideCategories = [...idOverride, ...(titleOverride?.categories || [])];
